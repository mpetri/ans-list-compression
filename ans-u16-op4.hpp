...
#include "ans-u16-util.hpp"
#include "util.hpp"

template <uint32_t t_block_size = 128, uint32_t t_frame_size = 262144>
struct ans_u16_op4 {
private:
    // clang-format off
    ...
    // clang-format on
    std::vector<ans_u16_encode_model<t_frame_size> > models;
    std::vector<ans_u16_decode_model<t_frame_size> > dmodels;
...
private:
        ...
        const t_model& m, const uint32_t* in, size_t n)
    {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            state = ans_u16_fake_encode(m, state, num, num_bytes);
        }
        return num_bytes;
    }
...
public:
    ...
    std::string name()
    {
        return "ans_u16_op4_M" + std::to_string(t_frame_size) + "_B"
    ...
    void init(const list_data& input, uint32_t* out, size_t& nvalue)
    {
        ...
        std::vector<u16_freq_table> freqs(thresholds.size());
        ...
        for (size_t i = 0; i < thresholds.size(); i++) {
            models.emplace_back(
                ans_u16_encode_model<t_frame_size>(freqs[i], true));
        }
        ...
        const uint32_t* in, const size_t len, uint32_t* out, size_t& nvalue)
    {
        ...
        for (size_t j = 0; j < num_blocks; j++) {
            ...
            for (size_t k = 0; k < block_size; k++) {
                ...
                state = ans_u16_encode(cur_model, state, num, out_ptr);
            }
            ans_u16_encode_flush(state, out_ptr);
        ...
        const uint32_t* in, const size_t len, uint32_t* out, size_t list_len)
    {
        ...
        for (size_t j = 0; j < num_blocks; j++) {
            ...
            auto state = ans_u16_decode_init(in8, enc_size);
            for (size_t k = 0; k < block_size; k++) {
