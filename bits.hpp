...
static const uint32_t lo_set[33] = {
    0x00000000ULL, 0x00000001ULL, 0x00000003ULL, 0x00000007ULL, 0x0000000FULL,
    0x0000001FULL, 0x0000003FULL, 0x0000007FULL, 0x000000FFULL, 0x000001FFULL,
    0x000003FFULL, 0x000007FFULL, 0x00000FFFULL, 0x00001FFFULL, 0x00003FFFULL,
    0x00007FFFULL, 0x0000FFFFULL, 0x0001FFFFULL, 0x0003FFFFULL, 0x0007FFFFULL,
    0x000FFFFFULL, 0x001FFFFFULL, 0x003FFFFFULL, 0x007FFFFFULL, 0x00FFFFFFULL,
    0x01FFFFFFULL, 0x03FFFFFFULL, 0x07FFFFFFULL, 0x0FFFFFFFULL, 0x1FFFFFFFULL,
    0x3FFFFFFFULL, 0x7FFFFFFFULL, 0xFFFFFFFFULL};
...
inline void write_int(uint32_t*& word, uint32_t x, uint8_t& offset,
                      const uint8_t len) {
    x &= lo_set[len & 63];
    if (offset + len < 32) {
        *word &=
...
struct bit_stream {
    ...
    bool write_mode;
    bit_stream(uint32_t* f, bool write_stuff) : of(f), write_mode(write_stuff) {
        first_ptr = &buf[0];
        cur_ptr = first_ptr;
        last_ptr = &buf[1];
        in_word_offset = 0;
