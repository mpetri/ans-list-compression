...
private:
    ...
    ans_byte_decode_model<t_frame_size> exception_model;

private:
    const uint8_t EXCEPT_MARKER = 0;
    const uint32_t VB_EXCEPT_THRES = t_block_size;
        ...
        const t_model& m, const uint32_t* in, size_t n)
    {
        ...
        uint32_t state = 0;
        std::array<uint32_t, t_block_size> exceptions;
        size_t num_except = 0;
        for (size_t i = 0; i < n; i++) {
            uint32_t num = in[n - i - 1];
            if (num > m.max_sym_encodeable) { // exception case!
                exceptions[num_except++] = num - (m.max_sym_encodeable + 1);
                num = EXCEPT_MARKER;
            }
        ...
        // encode exceptions
        if (num_except) {
            if (num_except > VB_EXCEPT_THRES) {
                using s16_codec = FastPForLib::Simple16<false>;
                static s16_codec s16;
                size_t enc_u32 = 0;
                s16.fakeencodeArray(exceptions.data(), num_except, enc_u32);
                num_bytes += (enc_u32 * sizeof(uint32_t));
            } else {
                for (size_t i = 0; i < num_except; i++) {
                    num_bytes += vbyte_size(exceptions[i]);
                }
            }
        }
...
public:
    ...
    std::string name()
    {
        return "ans_op4s_M" + std::to_string(t_frame_size) + "_B"
        ...
        const uint32_t* in, const size_t len, uint32_t* out, size_t& nvalue)
    {
        ...
        static std::array<uint32_t, t_block_size> exceptions;
        for (size_t j = 0; j < num_blocks; j++) {
            ...
            for (size_t k = 0; k < block_size; k++) {
                ...
                if (num > cur_model.max_sym_encodeable) { // exception case!
                    exceptions[num_except++]
                        = num - (cur_model.max_sym_encodeable + 1);
                    num = EXCEPT_MARKER;
                }
        ...
        const uint32_t* in, const size_t len, uint32_t* out, size_t list_len)
    {
        ...
        for (size_t j = 0; j < num_blocks; j++) {
            ...
            static std::array<uint32_t, t_block_size + 128> exceptions;
            static std::array<uint32_t, t_block_size> exception_pos;
            size_t num_except = 0;
            auto initblockout = out;
            for (size_t k = 0; k < block_size; k++) {
                ...
                if (num == EXCEPT_MARKER) {
                    exception_pos[num_except++] = k;
                }
                *out++ = num;
            }

            if (num_except) {
                // fprintff(stderr, "decoding %lu exceptions\n", num_except);
                // s16.decodeArray(in, enc_u32, out, list_len);
                if (num_except > VB_EXCEPT_THRES) {
                    using s16_codec = FastPForLib::Simple16<false>;
                    static s16_codec s16;
                    size_t enc_u32 = exceptions.size();
                    const uint32_t* in32 = (const uint32_t*)in8;
                    in8 = (const uint8_t*)s16.decodeArray(
                        in32, enc_u32, exceptions.data(), num_except);
                } else {
                    for (size_t i = 0; i < num_except; i++) {
                        exceptions[i] = vbyte_decode_u32(in8);
                    }
                }
                // fprintff(stderr, "read %lu bytes\n", (in8 - start));
                // place in the right spots
                for (size_t i = 0; i < num_except; i++) {
                    initblockout[exception_pos[i]]
                        = exceptions[num_except - i - 1]
                        + dmodel.max_sym_encodeable + 1;
                }
            }
        }
