...
template <bool MarkLength> class Simple16 {
  public:
...
void Simple16<MarkLength>::encodeArray(const uint32_t* in, const size_t length,
        ...
        if (trymefull<28, 1>(in)) {
            ...
            *out <<= 28 - 14 * NumberOfValuesCoded;
            ...
            if ((*in >> 28) > 0) {
                ...
                throw std::runtime_error("You tried to apply Simple16 to an "
                                         "incompatible set of integers.");
            }
            out[0] = 15;
            NumberOfValuesCoded = 1;
    ...
    while (ValuesRemaining > 0) {
        if (tryme<28, 1>(in, ValuesRemaining)) {
            out[0] = 0;
            NumberOfValuesCoded = (ValuesRemaining < 28)
            ...
            *out <<= 28 - NumberOfValuesCoded;
            ...
            out[0] = 1;
            NumberOfValuesCoded = (ValuesRemaining < 7)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 7;
            for (uint32_t i = 0; i < NumberOfValuesCoded; i++) {
                bit_writer(out, *in++, 2);
            ...
            *out <<= 28 - fill;
            ...
            out[0] = 3;
            NumberOfValuesCoded = (ValuesRemaining < 14)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 14;
            for (uint32_t i = 0; i < NumberOfValuesCoded; i++)
                bit_writer(out, *in++, 1);
            ...
            *out <<= 28 - 2 * NumberOfValuesCoded;
            ...
            out[0] = 5;
            NumberOfValuesCoded = (ValuesRemaining < 1)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 1;
            for (uint32_t i = 0; i < NumberOfValuesCoded; i++)
                bit_writer(out, *in++, 4);
            ...
            *out <<= 28 - 4 * NumberOfValuesCoded;
            ...
            out[0] = 8;
            NumberOfValuesCoded = (ValuesRemaining < 4)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 4;
            for (uint32_t i = 0; i < NumberOfValuesCoded; i++)
                bit_writer(out, *in++, 5);
            ...
            *out <<= 28 - 5 * base - 4 * NumberOfValuesCoded;
            ...
            out[0] = 9;
            NumberOfValuesCoded = (ValuesRemaining < 2)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 2;
            for (uint32_t i = 0; i < NumberOfValuesCoded; i++)
                bit_writer(out, *in++, 4);
            ...
            *out <<= 28 - 4 * base - 5 * NumberOfValuesCoded;
            ...
            out[0] = 10;
            NumberOfValuesCoded = (ValuesRemaining < 3)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 3;
            for (uint32_t i = 0; i < NumberOfValuesCoded; i++)
                bit_writer(out, *in++, 6);
...
void Simple16<MarkLength>::fakeencodeArray(const uint32_t* in,
    ...
    while (ValuesRemaining > 0) {
        if (tryme<28, 1>(in, ValuesRemaining)) {
            ...
            NumberOfValuesCoded = (ValuesRemaining < 9)
                                      ? static_cast<uint32_t>(ValuesRemaining)
                                      : 9;
        } else if (tryme<1, 3, 4, 4, 3, 3>(in, ValuesRemaining)) {
...
const uint32_t* Simple16<MarkLength>::decodeArray(const uint32_t* in,
    ...
    if (MarkLength)
        if ((*in) > nvalue) {
            quit("simple16: not enough storage.");
        }
...
size_t simple16_encode(const uint32_t* in, size_t n, uint32_t* out) {
    // (1) write length later
    uint32_t* output_len = out;
    out++;
    // (2) delegate to FastPFor code
    static Simple16<false> s16_coder;
    size_t written_u32 = n * 2;
    s16_coder.encodeArray(in, n, out, written_u32);
    *output_len = written_u32;
    return written_u32 + 1; // for encoding size and list len
}

size_t simple16_decode(const uint32_t* in, uint32_t* out, uint32_t list_len) {
    uint32_t encoding_u32 = *in++;
    static Simple16<false> s16_coder;
    size_t len = list_len;
    s16_coder.decodeArray(in, encoding_u32, out, len);
